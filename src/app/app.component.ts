...
export class AppComponent {
  ...
  constructor(
    ...
    this.user$ = authService.getUser$().pipe(shareReplay());
    ...
    this.loadingValuePerc$ = this.loaderService.loadingStatus$.pipe(
      ...
      switchMap(([prevLoadingStatus, loadingStatus]) => {
        if (
          prevLoadingStatus.total !== 0 &&
          loadingStatus.total === loadingStatus.finished
        ) {
          ...
          return of(
            loadingStatus.total
              ? ((loadingStatus.finished * 100) / loadingStatus.total).toFixed(
                  2,
                )
              : '',
          );
    ...
    this.userMenu$ = combineLatest([this.user$, this.pendingChanges$]).pipe(
          ...
          const userMenu: MenuItem[] = [
            ...
            },
            {
              label: pendingChanges
                ? `Changes to sync (${pendingChanges})`
                : 'No changes to sync',
              disabled: !pendingChanges,
              command: () =>
                this.spacedRepService.syncPendingChanges().subscribe(),
            },
            { separator: true },
          ];
