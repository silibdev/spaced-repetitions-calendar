...
import { debounceTime, defaultIfEmpty, distinctUntilChanged, forkJoin, map, Observable, switchMap, tap } from "rxjs";
...
export class SREventService {
  ...
  constructor(
    ...
    this.srViewerUIService.getFilter().pipe(
      untilDestroyed(this),
      debounceTime(250),
      ...
      switchMap(filter => this.loadSREvents(filter.date).pipe(
        map(events => {
